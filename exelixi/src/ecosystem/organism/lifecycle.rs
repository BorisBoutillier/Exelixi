...
pub struct OrganismBirth {
    ...
    // Chromosome for the new organism
...
pub fn organism_lifecycle(
    ...
    for OrganismBirth {
        ...
        energy,
        chromosome,
    } in std::mem::take(&mut lifecycle.births).into_iter()
    {
        let position =
            position.unwrap_or_else(|| ecosystem_config.environment.get_random_position(&mut *rng));
        let config = ecosystem_config
            .species
            .get(&species)
            .expect("Misconfigured species");
        let mut command = commands.spawn(position);
        let with_brain = config.eye.is_some();
        if with_brain {
            let individual = if chromosome.is_empty() {
        ...
        command.insert(Organism::new(config.id, chromosome));
